...
### 2.5 `scripts/check_warden.sh` (watchdog, cron-driven)
- 背景：cron `*/2 * * * *` 每 2 分鐘掃一次
- 若 pidfile 存在但 PID 已死 → 視為「非預期死亡」，自動重啟 (`start_warden.sh` 邏輯) + 寫 log
- 若 pidfile 不存在 → 不動作（可能使用者刻意不啟動）
- 由 2026-08-21 事件添加：warden 死 12 分鐘都沒人知，導致 Q&A 沒寫入
- log 格式：`[ISO] ⚠️  warden dead (pid=X, pidfile stale). Auto-restarting...` + `[ISO] ✅ warden auto-restarted, pid=Y`
- **測試**：`kill <pid> && bash scripts/check_warden.sh` 應自動重啟
...
# 手動 caption 一次 watchdog (驗證 script 送作)
bash ~/.openclaw/workspace-two/repos/agents-bulletin/scripts/check_warden.sh
...
# warden 死了怎麼辦？
# 1. 看 pidfile 是否還在: cat ~/.openclaw/agent-cowork/warden.pid
# 2. 如果在但 PID 沒了：等 watchdog (每 2 分鐘) 啟動，或手動 bash reload_warden.sh
# 3. 確認: ps -p $(cat ~/.openclaw/agent-cowork/warden.pid)
```
