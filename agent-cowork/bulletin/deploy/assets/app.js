...
(function () {
  ...
  function render(m) {
    ...
    const pending = m.pending_for_me || [];
    $("cnt-pending").textContent = pending.length;
    $("cnt-pending").classList.toggle("warn", pending.length > 0);
    ...
    const tbodyPEN = $("tbody-pending");
    if (pending.length === 0) {
      tbodyPEN.innerHTML = `<tr><td colspan="5" class="empty">沒有待回覆的 thread</td></tr>`;
    } else {
      ...
      const list = pending.map((id) => map.get(id)).filter(Boolean);
      tbodyPEN.innerHTML = list.map(pendingRowHTML).join("");
    }
  ...
  async function viewInit() {
    ...
    viewThreadId = tid;
    try {
      const r = await fetch(`${API.RAW}?id=${encodeURIComponent(tid)}`);
    ...
    if (submitBtn) {
      submitBtn.onclick = async () => {
        ...
        try {
          ...
          $("vp-status").textContent = "✓ 已送出，按 F5 看新內容";
          $("vp-text").value = "";
        } catch (e) {
    ...
    if (reloadLink) reloadLink.onclick = (e) => { e.preventDefault(); location.reload(); };
  }
